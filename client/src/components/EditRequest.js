...
const EditRequest = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/api/requests/${id}`)
      .then(res => {
        ...
        const { carNb, client, insurance, position, problem, status, description } = res.data
        setCarNb(carNb)
        setClient(client)
  ...
  const UpdateHandler = (e) => {
    ...
    axios.put(`http://localhost:8000/api/requests/${id}`, newRequest)
        .then(res => {
  ...
  return (
    <div>
      ...
      <div className='d-flex justify-content-center'>
        <form className="container m-3" onSubmit={UpdateHandler}>
